...
class EmployeeCalendarComponent extends StatefulWidget {
  ...
  @override
  State<EmployeeCalendarComponent> createState() => _EmployeeCalendarComponentState();
}
...
class _EmployeeCalendarComponentState extends State<EmployeeCalendarComponent> {
  ...
  Map<DateTime, bool> availableDatesMap = {};
  ...
  Future<void> fetchAvailableDates() async {
    ...
    try {
      final uri = Uri.parse(
        'https://hairmake-grow.com/web-booking?page=api_date&method=view&branch_id=${widget.branchId}&staff_id=${widget.employeeId}',
      );
      ...
      if (res.statusCode == 200) {
        ...
        data.forEach((key, value) {
          availableDatesMap[DateTime.parse(key)] = value == 1;
        });

        if (selectedDate == null && widget.onSelect != null) {
          final firstAvailable = availableDatesMap.entries.firstWhere(
                (e) => e.value && !e.key.isBefore(DateTime.now()),
            orElse: () => MapEntry(DateTime.now(), false),
          );
          if (firstAvailable.value) {
            _onSelect(firstAvailable.key);
          }
        }
      } else {
        apiError = "Server error: ${res.statusCode}";
      }
    } catch (e) {
      apiError = e.toString();
    }
  ...
  bool _isAvailableApi(DateTime date) {
    return availableDatesMap[DateTime(date.year, date.month, date.day)] ?? false;
  }

  String? _getBranchName(DateTime date, List<EmployeeWorkingDayModel> snap) {
    final index = widget.disableOtherBranchsDate
        ? snap.indexWhere((e) => e.date.isSameDateWith(date) && e.branchId == widget.branchId)
  ...
  bool _isAvailable(DateTime date, List<EmployeeWorkingDayModel> snap) {
    return widget.disableOtherBranchsDate
        ? snap.any((e) => e.date.isSameDateWith(date) && e.branchId == widget.branchId)
        : snap.any((e) => e.date.isSameDateWith(date));
  }
  ...
  Widget build(BuildContext context) {
    if (isLoadingApi) return const TableCalendarShimmer();
    ...
    return SnapHelperWidget<List<EmployeeWorkingDayModel>>(
      ...
      onSuccess: (workingDays) {
        if (selectedDate == null && widget.onSelect != null && workingDays.isNotEmpty) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        return TableCalendar(
          ...
          calendarBuilders: CalendarBuilders(
            defaultBuilder: (context, date, _) {
              ...
              final isAvailableSchedule = _isAvailable(date, workingDays);
              final isAvailable = !isPast && isAvailableSchedule && _isAvailableApi(date);
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // ✅ Show branch name ONLY if available
                      if (isAvailable)
                        Text(
                          branchName.validate(),
                          style: secondaryTextStyle(
